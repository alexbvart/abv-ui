...
const Card: FC<CardProps> & CompoundComponents = ({
  ...
  return (
    <StyledCard {...rest}>
      <ContentContainer className={clsx(className)}  >{children}</ContentContainer>
    </StyledCard>
  );
};
...
const ImageContainer = styled.div<{ imagePosition?: ImagePosition, isFull?: boolean  }>`
  top: 0;
  left: 0;
  width: 100%;
  overflow: hidden;
...
const Image = styled.img<{ imagePosition?: ImagePosition, isFull?: boolean }>`
  ...
  ${(props) =>
  !props.isFull && ["top", "bottom","center"].includes(props.imagePosition || "") &&
    css`
      max-height: 200px; 
      border-radius: 8px;
      object-fit: cover;
...
const CardImage: FC<CardImageProps> = ({
  ...
  <ImageContainer imagePosition={imagePosition} isFull={isFull}  className={className}>
    <Image src={image} alt={alt} isFull={isFull} imagePosition={imagePosition}/>
  </ImageContainer>
);
